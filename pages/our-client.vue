        ...
        <table class="min-w-full bg-white  shadow-md rounded-lg overflow-hidden">
          <thead>
            <tr class="bg-gray-200">
              ...
              <th class="px-4 py-2 cursor-pointer" @click="sortTable('email')">Logo</th>
          ...
          <tbody>
            <tr class="text-center" v-for="(item, index) in sortedData" :key="index">
              ...
              <td class="px-4 py-2 flex justify-center"><img class="w-10 h-10 " :src="item.s3_url"/></td>
...
const getAllUsers = async () => {
  try {
    const response = await $axios.get('/our_clients'); // Make a GET request to the API
